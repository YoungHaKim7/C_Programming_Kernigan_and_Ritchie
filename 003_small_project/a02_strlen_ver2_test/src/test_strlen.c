...
#define TEST_STRLEN(str, expected_len) do { \
    char buffer[256]; \
    ...
    printf("  Builtin:  %zu\n", builtin_len); \
    \
    TEST_ASSERT(simple_len == expected_len, "Simple implementation correct"); \
    TEST_ASSERT(glibc_len == expected_len, "Glibc implementation correct"); \
    TEST_ASSERT(builtin_len == expected_len, "Builtin implementation correct"); \
...
void test_performance() {
    ...
    const char* test_string = "This is a performance test string with moderate length";
...
void test_alignment_stress() {
    ...
    char* aligned_ptr = (char*)(((uintptr_t)aligned_buffer + 7) & ~7); // 8-byte aligned
