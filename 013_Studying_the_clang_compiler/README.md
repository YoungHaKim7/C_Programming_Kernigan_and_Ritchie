...
- The `-lm` flag tells the linker to include the C math library (`libm`). The `-l` part means "link library", and `m` stands for "math". You need this flag whenever your code uses math functions like `sqrt()`, `sin()`, or `pow()` from `<math.h>`.
...
OPTIONS:
  ...
  -falloc-token-max=<N>   Limit to maximum N allocation tokens (0 = target SIZE_MAX)
  ...
  -fdefer-ts              Enable support for the C '_Defer' Technical Specification
  ...
  -fmatrix-memory-layout=<value>
                          Sets the matrix memory layout (row-major or column-major)
  ...
  -fno-partition-static-data-sections
                          Disable partition static data sections using profile information (x86 and aarch64 ELF)
  ...
  -fpartition-static-data-sections
                          Enable partition static data sections using profile information (x86 and aarch64 ELF)
  ...
  -gcall-site-info        Enable call site debug info
  ...
  -gdwarf-6               Generate source-level debug information with DWARF version 6
  ...
  -gno-call-site-info     Disable call site debug info
  ...
  -Xarch_device <arg>     Pass <arg> to device compilation in the offloading toolchain
  -Xarch_host <arg>       Pass <arg> to host compilation in the offloading toolchain
