...
fmt_flags := if os == "Linux" { \
    ...
    ". -regex '.*\\.\\(cpp\\|hpp\\|cc\\|cxx\\|c\\|m\\|mm\\|h\\)' -exec " \
    +clang_format+ \
    " -style=file -i {} \\;" \
  }  
...
# (Objective-C)clang compile(macOS/LinuxOS)
r:
	just fm
	rm -rf {{target_dir}}
	mkdir -p {{target_dir}}
	{{clang_which}} {{ldflags_common}} -o {{target_dir}}/{{project_name}} {{source}}
	{{target}}
...
cr:
	...
	rm -rf build
	mkdir -p build
	export CC={{clang_which}}
	cmake -D CMAKE_C_COMPILER={{clang_which}} -G Ninja .
	ninja
...
cro:
	...
	cmake -D CMAKE_BUILD_TYPE=RelWithDebInfo \
	      -D CMAKE_C_COMPILER={{clang_which}} \
