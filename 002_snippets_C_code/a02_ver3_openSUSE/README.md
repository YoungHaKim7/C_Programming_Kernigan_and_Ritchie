...
Done. The justfile now dynamically detects the best available clang version on Linux:

**Changes made:**

1. **`clang_which`** (compiler): Searches for clang-21 through clang-30, uses the first found, falls back to `clang`

2. **`clangpp_which`** (C++ compiler): Searches for clang++-21 through clang++-30, uses the first found, falls back to `clang++`

3. **`clang_format`** (formatter): Searches for clang-format-21 through clang-format-30, uses the first found, falls back to `clang-format`

The search order is `clang-21` → `clang-22` → ... → `clang-30` → `clang` (system default). This ensures you get the highest version 21+ that's installed, or the system default if none are found.

- Done. The `gcc_which` variable now:

1. **On Linux**: Tries these paths in order:
   - `/opt/gcc-15/bin/gcc`
   - `/usr/bin/gcc-15`
   - Falls back to `clang-15` through `clang-30` (first found)
   - Finally defaults to system `gcc`

2. **On macOS**: Tries:
   - `/opt/homebrew/opt/gcc@15/bin/gcc-15`
   - Falls back to `llvm@15` through `llvm@30` (first found)
   - Finally defaults to `/opt/homebrew/opt/llvm/bin/clang`

3. **Other OS**: Uses system `gcc`

On your system with clang-21 available, it will use `/usr/bin/clang-21` if gcc-15 isn't found at the specified paths.
