...
void parse_field_options(char *opt_str, SortOptions *opts) {
    ...
    while (token && opts->field_count < MAXFIELDS) {
        ...
        for (int i = 0; token[i]; i++) {
            switch (token[i]) {
                case 'n': opts->field_opts[opts->field_count].numeric = 1; break;
                case 'r': opts->field_opts[opts->field_count].reverse = 1; break;
                case 'f': opts->field_opts[opts->field_count].fold = 1; break;
                case 'd': opts->field_opts[opts->field_count].directory = 1; break;
            }
        }
        opts->field_count++;
        token = strtok(NULL, ",");
